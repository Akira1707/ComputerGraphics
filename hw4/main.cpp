...
bool LoadWICImage(const wchar_t* filename, TextureDesc& desc)
{
    ...
    UINT32 dataSize = desc.pitch * desc.height;
    desc.pData = malloc(dataSize);
    if (!desc.pData)
    {
        SAFE_RELEASE(converter);
    ...
    hr = converter->CopyPixels(nullptr, desc.pitch, dataSize, reinterpret_cast<BYTE*>(desc.pData));
    if (FAILED(hr))
    {
        free(desc.pData);
        desc.pData = nullptr;
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPWSTR, _In_ int nCmdShow)
{
    ...
    g_hWnd = CreateWindowW(wc.lpszClassName, L"Thu Hoai - Cube + Skybox",
...
void RenderFrame()
{
    ...
    HRESULT hr = g_pDeviceContext->Map(g_pViewProjBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSky);
    if (SUCCEEDED(hr))
    {
    ...
    rsDescCube.DepthClipEnable = TRUE;
