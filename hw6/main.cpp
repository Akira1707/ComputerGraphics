...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPWSTR, _In_ int nCmdShow)
{
    ...
    wc.lpszClassName = L"DX11Lighting";
    ...
    g_hWnd = CreateWindowW(
        wc.lpszClassName,
        L"Thu Hoai - Point Lights + Normal Map",
        WS_OVERLAPPEDWINDOW,
...
float4 ps(VSOutput pixel) : SV_Target
{
    ...
    for (int i = 0; i < lightCount.x; ++i)
    {
        ...
        finalColor += albedo * diffuse * lightColor * attenuation;
        finalColor += spec * lightColor * attenuation * 0.35f;
    }
...
// Texture helpers
bool CreateTextureFromDesc(const TextureDesc& texDesc, ID3D11Texture2D** ppTex, ID3D11ShaderResourceView** ppSRV)
{
    ...
    HRESULT hr = g_pDevice->CreateTexture2D(&tex2DDesc, texData.data(), ppTex);
    if (FAILED(hr) || !*ppTex)
        return false;
...
void LoadTextures()
{
    ...
    {
        TextureDesc texDesc;
        if (!LoadImageAny(L"cube.dds", texDesc))
        {
            MessageBoxA(NULL, "Failed to load cube.dds", "Error", MB_OK);
            return;
        }

        if (!CreateTextureFromDesc(texDesc, &g_pTexture, &g_pTextureView))
        {
            free(texDesc.pData);
            MessageBoxA(NULL, "Failed to create diffuse texture", "Error", MB_OK);
            return;
        }
...
void RenderFrame()
{
    ...
    sceneData.ambientColor = XMFLOAT4(0.12f, 0.12f, 0.14f, 1.0f);
    sceneData.lightCount = XMINT4(2, 0, 0, 0);

    sceneData.lights[0].position = XMFLOAT4(1.8f * cosf(angle), 1.2f, 1.8f * sinf(angle), 1.0f);
    sceneData.lights[0].color = XMFLOAT4(1.0f, 0.82f, 0.72f, 1.0f);

    sceneData.lights[1].position = XMFLOAT4(-1.5f, 0.8f + 0.3f * sinf(angle * 1.4f), -1.2f, 1.0f);
    ...
    ID3D11ShaderResourceView* cubeSRVs[] = { g_pTextureView, g_pNormalTextureView };
    g_pDeviceContext->PSSetShaderResources(0, 2, cubeSRVs);

    ID3D11Buffer* cubeCBs[] = { g_pModelBuffer, g_pViewProjBuffer, g_pSceneBuffer };
    g_pDeviceContext->VSSetConstantBuffers(0, 2, cubeCBs);
    g_pDeviceContext->PSSetConstantBuffers(2, 1, &g_pSceneBuffer);

    // Cube 1
    {
        XMMATRIX model1 = XMMatrixRotationY(angle);
        XMMATRIX normal1 = XMMatrixTranspose(XMMatrixInverse(nullptr, model1));

        ModelBuffer mb = {};
        XMStoreFloat4x4((XMFLOAT4X4*)&mb.model, XMMatrixTranspose(model1));
        XMStoreFloat4x4((XMFLOAT4X4*)&mb.normalMatrix, XMMatrixTranspose(normal1));
        g_pDeviceContext->UpdateSubresource(g_pModelBuffer, 0, nullptr, &mb, 0, 0);

        ID3D11Buffer* vsCBs[] = { g_pModelBuffer, g_pViewProjBuffer };
        g_pDeviceContext->VSSetConstantBuffers(0, 2, vsCBs);
    ...
    // Cube 2
    {
        XMMATRIX model2 =
            ...
            XMMatrixRotationX(angle * 0.7f) *
            XMMatrixTranslation(1.6f, 0.1f, 0.8f);

        XMMATRIX normal2 = XMMatrixTranspose(XMMatrixInverse(nullptr, model2));

        ModelBuffer mb = {};
        XMStoreFloat4x4((XMFLOAT4X4*)&mb.model, XMMatrixTranspose(model2));
        XMStoreFloat4x4((XMFLOAT4X4*)&mb.normalMatrix, XMMatrixTranspose(normal2));
        g_pDeviceContext->UpdateSubresource(g_pModelBuffer, 0, nullptr, &mb, 0, 0);
